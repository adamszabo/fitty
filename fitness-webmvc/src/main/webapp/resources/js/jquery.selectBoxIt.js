					...
					.widget(
							"selectBox.selectBoxIt",
							{
								...
								_create : function() {
									...
									// box
									self.firstSelectItem = self.element.find(
											"option").slice(0, 1);
									...
									if (self.options["theme"] === "bootstrap") {
										...
										// Adds Twitter Bootstrap classes to the
										// dropdown list
										...
										// Adds jQueryUI classes to the dropdown
										// list
										self._jqueryui();
										...
										// Adds regular classes to the dropdown
										// list
										self._addClasses();
								...
								_createdropdown : function() {
									...
									// Appends the default text to the inner
								...
								_createUnorderedList : function() {
											...
											.each(function(index) {
												...
												if ($(this).parent().is(
													...
													if ($(this).index() === 0) {
														...
														optgroupElement = '<span class="selectboxit-optgroup-header" data-disabled="true">'
																+ $(this)
																		.parent()
																		.first()
																		.attr(
												...
												currentItem += optgroupElement
														...
														+ ($(this)
																.attr("class") || "")
														+ '"><a class="selectboxit-option-anchor"><i class="selectboxit-option-icon '
														+ iconClass
														+ ' '
														+ iconUrlClass
														+ '"'
									...
									if (self.options["defaultText"]) {
										...
										self.dropdownText
												.text(self.options["defaultText"]);
									}

									// If the `defaultText` HTML5 data attribute
									// is being used
									if (self.selectBox.data("text")) {

										// Overrides the current dropdown
										// default text with the value from the
										// HTML5 `defaultText` value
										self.dropdownText.text(self.selectBox
												.data("text"));

										self.options["defaultText"] = self.selectBox
												.data("text");
									...
									// Stores the indropdownidual dropdown list
									...
									self.list.find("li[data-disabled='true']")
											.not(".optgroupHeader").addClass(
													self.disabledClasses);

									// If the first select box option is
									// disabled, and the user has chosen to not
									// show the first select box option
									if (self.currentFocus === 0
											&& !self.options["showFirstOption"]
											&& self.listItems.eq(0).hasClass(
													self.disabledClasses)) {

										// Sets the default value of the
										// dropdown list to the first option
										// that is not disabled
										self.currentFocus = +self.listItems
								...
								_replaceSelectBox : function() {
									...
									// dropdown list
									self.dropdown
											.append(this.options["nostyle"] ? self.downArrow
													: self.downArrowContainer);

									if (!self.options["nostyle"]) {

										// The dynamic CSS of the down arrow
										// container element
										...
										// `line-height` CSS styles of the
										// dropdown list text element
										self.dropdownText
												.css({

													"line-height" : self.dropdown
															.css("height"),

													"max-width" : self.dropdown
															.outerWidth()
															- (self.downArrowContainer
																	.outerWidth() + self.dropdownImage
																	.outerWidth())
										...
										// name to the currently selected drop
										// down option
								...
								_scrollToView : function(type) {
									...
									// dropdown list option
									currentItemHeight = self.listItems.eq(
											self.currentFocus).height(),

									// The relative distance from the currently
									// selected dropdown list option to the the
									// top of the dropdown list options list
									currentTopPosition = self.listItems.eq(
											self.currentFocus).position().top,
								...
								open : function(callback) {
									...
									if (self._dynamicPositioning) {
										...
										// Dynamically positions the dropdown
										// list options list
									...
									if (!this.list.is(":visible")) {
										...
										// when opening the dropdown list
										// options list
										...
										// Uses the jQuery `slideDown` special
										// effect
										case "slideDown":

											// Requires a callback function to
											// determine when the `slideDown`
											// animation is complete
											self.list
													.slideDown(
															self.options["showEffectSpeed"],
										...
										// Uses the jQuery `fadeIn` special
										// effect
										case "fadeIn":
											...
											// `scrollToView`
											self.list
													.fadeIn(self.options["showEffectSpeed"]);
										...
										default:
											...
											self.list
													.show(
															self.options["showEffect"],
															self.options["showEffectOptions"],
															self.options["showEffectSpeed"],
															function() {
								...
								close : function(callback) {
									...
									if (self.list.is(":visible")) {
										...
										// Determines what jQuery effect to use
										// when closing the dropdown list
										// options list
										...
										case "hide":
											...
											self.list
													.hide(self.options["hideEffectSpeed"]);

											break;

										// Uses the jQuery `slideUp` special
										// effect
										case "slideUp":

											self.list
													.slideUp(self.options["hideEffectSpeed"]);

											break;

										// Uses the jQuery `fadeOut` special
										// effect
										case "fadeOut":

											self.list
													.fadeOut(self.options["hideEffectSpeed"]);
										...
										default:
											...
											self.list
													.hide(
															self.options["hideEffect"],
															self.options["hideEffectOptions"],
															self.options["hideEffectSpeed"],
															function() {
								...
								_eventHandlers : function() {
									...
									self.list
											...
											// Select box indropdownidual
											// options events bound with the
											// jQuery `delegate` method.
											// `Delegate` was used because
											// binding indropdownidual events to
											// each list item (since we don't
											// know how many there will be)
											// would decrease performance.
											// Instead, we bind each event to
											// the unordered list, provide the
											// list item context, and allow the
											// list item events to bubble up
											// (`event bubbling`). This greatly
											// increases page performance
											// because we only have to bind an
											// event to one element instead of x
											// number of elements. Delegates the
											// `click` event with the
											// `selectBoxIt` namespace to the
											// list items
											.delegate(
													"li",
													"click.selectBoxIt",
											...
											// Delegates the `focus` event with
											// the `selectBoxIt` namespace to
											// the list items
											.delegate(
													"li",
													"focus.selectBoxIt",
								...
								_update : function(elem) {
									...
									if (elem.attr("data-disabled") === "false") {
										...
										// disabled
										if ((self.options["defaultText"] && self.dropdownText
												.text() === self.options["defaultText"])) {
								...
								_addClasses : function(obj) {
									...
									focusClass = obj.focusClasses
											|| "selectboxit-focus",

									hoverClass = obj.hoverClasses
											|| "selectboxit-hover",

									buttonClass = obj.buttonClasses
											|| "selectboxit-btn",

									listClass = obj.listClasses
									...
									self.listItems.bind({
										...
										// `focus` event with the `selectBoxIt`
										// namespace
										"focus.selectBoxIt" : function() {

											// Adds the focus CSS class to the
											// currently focused dropdown list
											// option
										...
										// `blur` event with the `selectBoxIt`
										// namespace
										"blur.selectBoxIt" : function() {

											// Removes the focus CSS class from
											// the previously focused dropdown
											// list option
									...
									self.listItems
											.bind({
												...
												"mouseenter.selectBoxIt" : function() {
													...
													// If the currently moused
													// over drop down option is
													// not disabled
													if ($(this).attr(
															"data-disabled") === "false") {
								...
								_jquerymobile : function() {
									...
									var self = this, theme = self.selectBox
											.attr("data-theme")
											|| "c";
									...
									self
											._addClasses({
												...
												focusClasses : "ui-btn-down-"
														+ theme,

												hoverClasses : "ui-btn-hover-"
														+ theme,

												arrowClasses : "ui-icon ui-icon-arrow-d ui-icon-shadow",

												buttonClasses : "ui-btn ui-btn-icon-right ui-btn-corner-all ui-shadow ui-btn-up-"
														+ theme,

												listClasses : "ui-btn ui-btn-icon-right ui-btn-corner-all ui-shadow ui-btn-up-"
														+ theme,
								...
								destroy : function(callback) {
									...
									// Calls the jQueryUI Widget Factory destroy
									// method
								...
								_destroySelectBoxIt : function() {
									...
									// Unbinds all of the dropdown list event
									// handlers with the `selectBoxIt` namespace
									self.dropdown.unbind(".selectBoxIt").

									// Undelegates all of the dropdown list
									// event handlers with the `selectBoxIt`
									// namespace
									undelegate(".selectBoxIt");

									// Remove all of the `selectBoxIt` DOM
									// elements from the page
									self.dropdownContainer.remove();

									// Triggers the custom `destroy` event on
									// the original select box
								...
								// Apply Native Select
								// -------------------
								// The dropdown will use the native select box
								// functionality

								_applyNativeSelect : function() {

									var self = this, currentOption;
									...
									// Positions the original select box
									// directly over top the new dropdown list
									// using position absolute and "hides" the
									// original select box using an opacity of
									// 0. This allows the mobile browser "wheel"
								...
								// ----------------------
								// Add's all attributes (excluding id, class
								// names, and the style attribute) from the
								// default select box to the new drop down
								_addSelectBoxAttributes : function(eventName) {

									var self = this,
									// Get's all of the properties associated
									// with the select box and turns it into an
									// array
									selectBoxProperties = (function() {

										var arr = [], x, attrs = self.selectBox
												.prop("attributes");
									...
									// Add's all attributes to the currently
									// traversed drop down option
									self._addAttributes(selectBoxProperties,
											self.dropdown);

									// Add's all attributes to the drop down
									// items list
									self.selectItems
											.each(function(iterator) {

												var arr = [], x, attrs = $(this)
														.prop("attributes");

												// Get's all of the properties
												// associated with currently
												// traversed select box option
												...
												// Add's all attributes to the
												// currently traversed drop down
												// option
												self._addAttributes(
														optionProperties,
														self.listItems
...
// Immediately-Invoked Function Expression (IIFE) [Ben Alman Blog
// Post](http://benalman.com/news/2010/11/immediately-invoked-function-expression/)
// that calls another IIFE that contains all of the plugin logic. I used this
// pattern so that anyone viewing this code would not have to scroll to the
// bottom of the page to view the local parameters that were passed to the main
// IIFE.
...
// passed in locally, to improve performance, since javascript first searches
// for a variable match within the local variables set before searching the
// global variables set. All of the global variables are also passed in locally
// to be minifier friendly. `undefined` can be passed in locally, because it is
// not a reserved word in JavaScript.
...
(function($, window, document, undefined) {
	...
	$.selectBox.selectBoxIt.prototype._ariaAccessibility = function() {
		...
		self.dropdown.attr(
				{
					...
					// W3C `aria-activedescendant` description: This is used
					// when a composite widget is responsible for managing its
					// current active child to reduce the overhead of having all
					// children be focusable. Examples include: multi-level
					// lists, trees, and grids.
					"aria-activedescendant" : self.listItems.eq(
							self.currentFocus).attr("id"),

					// W3C `aria-label` description: It provides the user with a
					// recognizable name of the object.
					"aria-label" : $(
							"label[for='" + self.originalElem.id + "']").text()
							|| "",

					// W3C `aria-live` description: Indicates that an element
					// will be updated.
					// Use the assertive value when the update needs to be
					// communicated to the user more urgently.
					"aria-live" : "assertive"
				}).

		// Dynamically adds `ARIA attributes` if the new dropdown list is
		// enabled or disabled
		bind({
			...
			"disable.selectBoxIt" : function() {
				...
				// W3C `aria-disabled` description: Indicates that the element
				// is perceivable but disabled, so it is not editable or
				// otherwise operable.
		...
		self.list.attr({
			...
			// W3C `listbox` description: A widget that allows the user to
		...
		self.listItems.attr({
			...
			// This must be set for each element when the container element role
			// is set to `listbox`
			"role" : "option"
		});

		// Dynamically updates the new dropdown list `aria-label` attribute
		// after the original dropdown list value changes
		self.selectBox.bind({
			...
			"open.selectBoxIt" : function() {
				...
				// Indicates that the dropdown list options list is currently
				// visible
		...
		(function($, window, document, undefined) {
			...
			$.selectBox.selectBoxIt.prototype.disable = function(callback) {
				...
				if (!self.options["disabled"]) {
					...
					// Calls the jQueryUI Widget Factory disable method to make
					// sure all options are correctly synced
			...
			$.selectBox.selectBoxIt.prototype.disableOption = function(index,
					callback) {
	...
	selectBoxIt(window.jQuery, window, document);
		...
		(function($, window, document, undefined) {
			...
			$.selectBox.selectBoxIt.prototype._dynamicPositioning = function() {
				...
				// Returns the x and y coordinates of the dropdown list options
				// list relative to the document
				...
				topToBottom = (listOffsetTop + selectBoxHeight + listHeight <= windowHeight
						+ windowScrollTop),
				...
				// If there is room on the bottom of the viewport to display the
				// drop down options
				if (!bottomReached) {
				...
				// If there is not enough room on the top or the bottom
					...
					outsideTopViewport = Math
							.abs((self.dropdown.offset().top - windowScrollTop)
									- listHeight);
					...
					if (outsideBottomViewport < outsideTopViewport) {
						...
						self.list
								.css("max-height", self.list.data("max-height")
					...
					// If there is more room on the top
						...
						self.list.css("max-height", self.list
								.data("max-height")
								- outsideTopViewport - (selectBoxHeight / 2));
...
(function(selectBoxIt) {
	...
	selectBoxIt(window.jQuery, window, document);
		...
		(function($, window, document, undefined) {
			...
			$.selectBox.selectBoxIt.prototype.enableOption = function(index,
					callback) {
...
(function($, window, document, undefined) {
	...
	$.selectBox.selectBoxIt.prototype.moveDown = function(callback) {
		...
		// Increments `currentFocus`, which represents the currently focused
		// list item `id` attribute.
		self.currentFocus += 1;

		// Determines whether the dropdown option the user is trying to go to is
		// currently disabled
		var disabled = self.listItems.eq(self.currentFocus).attr(
				"data-disabled") === "true" ? true : false,

		hasNextEnabled = self.listItems.eq(self.currentFocus).nextAll("li")
		...
		// If the option the user is trying to go to is disabled, but there is
		...
		// If the user has not reached the bottom of the unordered list
			...
			// The jQuery `end()` method allows you to continue chaining while
			// also using a different selector
			self.listItems.eq(self.currentFocus - 1).blur().end().

			// Focuses the currently focused list item
			eq(self.currentFocus).focus();

			// Calls `scrollToView` to make sure the `scrollTop` is correctly
			// updated. The `down` user action
	...
	$.selectBox.selectBoxIt.prototype.moveUp = function(callback) {
		...
		hasPreviousEnabled = self.listItems.eq(self.currentFocus).prevAll("li")
				.not("[data-disabled='true']").first().length;
		...
		// If the option the user is trying to go to is disabled and the user is
		// not trying to go up after the user has reached the top of the list
...
(function(selectBoxIt) {
	...
	selectBoxIt(window.jQuery, window, document);
		...
		(function($, window, document, undefined) {
			...
			$.selectBox.selectBoxIt.prototype._setCurrentSearchOption = function(
					currentOption) {
			...
			$.selectBox.selectBoxIt.prototype._searchAlgorithm = function(
					currentIndex, alphaNumeric) {
				...
				for (x = currentIndex, arrayLength = self.textArray.length; x < arrayLength; x += 1) {
					...
					// options text if the currentText < 2 characters
					if (self.currentText.length < 3) {

						alphaNumeric = new RegExp(self.currentText.charAt(0),
								"gi");
					...
					if (self.textArray[x].toLowerCase() === self.currentText
							.toLowerCase()) {
			...
			$.selectBox.selectBoxIt.prototype.search = function(
					alphaNumericKey, rememberPreviousSearch, callback) {
				...
				// Wraps the current user text search in a regular expression
				// that is case insensitive and searches globally
				var alphaNumeric = new RegExp(self.currentText, "gi"),

				// Calls `searchAlgorithm` which searches an array that contains
				// all of the dropdown list option values.
				notFound = self._searchAlgorithm(self.currentIndex,
						alphaNumeric);
...
(function($, window, document, undefined) {
	...
	$.selectBox.selectBoxIt.prototype.selectOption = function(val, callback) {
		...
		if (typeof val === "number") {
			...
			// Set's the original select box value and triggers the change event
			// (which SelectBoxIt listens for)
			self.selectBox.val(self.selectBox.find("option").eq(val).val())
					.change();
...
(function(selectBoxIt) {
	...
	selectBoxIt(window.jQuery, window, document);
		...
		(function($, window, document, undefined) {
			...
			$.selectBox.selectBoxIt.prototype.setOption = function(key, value,
					callback) {
				...
				// If a user sets the `showFirstOption` to true
					...
					self.dropdownImage.attr("class", value
							+ " selectboxit-arrow");
...
(function($, window, document, undefined) {
	...
	$.selectBox.selectBoxIt.prototype.setOptions = function(newOptions,
			callback) {
		...
		if (self.options["defaultIcon"]) {
			...
			self.dropdownImage.attr("class", self.options["defaultIcon"]
					+ " selectboxit-arrow");
		...
		// If the defaultText option is set, make sure the dropdown list default
		// text reflects this value
		if (self.options["defaultText"]) {
...
(function($, window, document, undefined) {
	...
	$.selectBox.selectBoxIt.prototype.wait = function(time, callback) {
		...
		// The timeout variable stores a Deferred Object, which will be resolved
	...
	$.selectBox.selectBoxIt.prototype.returnTimeout = function(time) {
		...
		return $.Deferred(function(dfd) {
			...
			// Call the JavaScript `setTimeout function and resolve the Deferred
			// Object
